...
### Méthode 1 : Communiquer les demandes d’exclusion de la vente du CCPA à l’aide d’un segment [!UICONTROL CCPA Opt-Out-of-Sale] dans Advertising Cloud DSP
...
1. [Créez un segment d’opposition à la vente des informations personnelles (CCPA) et implémentez le pixel de segment pour capturer les demandes](/help/dsp/audiences/ccpa-opt-out-segment-create.md) d’opposition.
...
1. Utilisez l’API Adobe Experience Platform Privacy Service pour [soumettre des demandes d’opposition à la vente](https://experienceleague.adobe.com/docs/experience-platform/privacy/api/consent.html) à Advertising Cloud pour le compte des consommateurs et pour vérifier le statut des demandes existantes.
...
Vous pouvez récupérer les liens vers les rapports mensuels créés au cours des trois mois précédents, soit depuis Advertising Cloud DSP, soit à l’aide de la fonction Advertising Cloud [!DNL Trafficking API]. Chaque lien est valide pendant sept jours, mais est actualisé chaque fois qu’un client tente d’en récupérer un.
...
### Méthode 2 : Récupérer les rapports d’exclusion de la vente des consommateurs à l’aide d’Advertising Cloud [!DNL Trafficking API]

Cette fonctionnalité est disponible pour les organisations qui utilisent [!DNL Trafficking API]. Pour plus d’informations, consultez la documentation de [!DNL Trafficking API] .

Si votre entreprise n’utilise pas la balise [!DNL Trafficking API] mais souhaite obtenir plus d’informations, contactez votre gestionnaire de compte d’Adobe.

## Annexe : Exemple [!UICONTROL CCPA Opt-Out-of-Sale] Requête pour les utilisateurs de l’API de Privacy Service
...
* `"namespace": "AdCloud"` indique l’espace du  `AdCloud` cookie et la valeur correspondante est l’ID de cookie du client tel qu’il est récupéré dans  `AdobePrivacy.js`
