...
Il est important de comprendre si l&#39;objectif de la campagne est d&#39;atteindre le RSDP le plus élevé possible ou le CPA le plus faible possible. Pour chaque kit de l&#39;opération, vous indiquez l&#39;objectif visé en conséquence, soit *[!UICONTROL Highest ROAS - Custom Goal]* soit *[!UICONTROL Lowest CPA - Custom Goal]*.
...
* Utilisez l’exécution du ciblage réseau (RON) : Il est important d’inclure une série de placement réseau sans ciblage d’audience et avec un ciblage d’inventaire étendu. Cela permet à l’algorithme [!DNL Adobe Sensei] de rechercher des utilisateurs de valeur qui peuvent avoir des cookies plus récents qui n’ont pas encore été classés dans une audience.
...
* **Traitement et limitation :** pour sélectionner un objectif d’optimisation du CPA ou du RSDP, le package doit utiliser un traitement au niveau du package. Cela permet de s’assurer que tous les emplacements du module sont optimisés pour répartir les dépenses selon les performances et l’échelle en fonction des objectifs sélectionnés.

* **Dates de vol :**  (forfaits de prospection) lorsque votre campagne s’exécute pendant plus de 25 jours, utilisez la  [!UICONTROL Activate Custom Flighting] fonctionnalité. Tout d’abord, définissez un vol personnalisé pour les 10 premiers jours à environ 75 % du budget quotidien nécessaire pour réduire les dépenses pendant la *phase d’apprentissage*. Définissez ensuite un second vol personnalisé pour le reste du budget.
...
* **Budget :** DSP tentera toujours d’allouer 100 % du budget du package de manière égale entre tous les emplacements d’un package. Si un emplacement a de faibles dépenses ou pas de dépenses, nous vous recommandons de limiter le budget de l’emplacement afin de permettre une plus grande partie du budget à affecter aux emplacements à l’échelle. Prévoyez de 24 à 48 heures pour que les modifications du budget soient calibrées.
...
* **Objectifs personnalisés :**
   ...
   * Saisissez la [!UICONTROL Target CPA] ou [!UICONTROL Target ROAS] appropriée.

* **Stratégie de remplissage de l’espacement :** sélectionnez cette option  *[!UICONTROL Even]* pour optimiser vos objectifs de performances en effectuant un espacement uniforme tout au long de la date de vol.

   Utilisez le rythme *[!UICONTROL FrontLoad]* et *[!UICONTROL Aggressive Front Load]* uniquement lorsque vous priorisez complètement la diffusion et que vous dépensez plus que l’optimisation des performances, car ces stratégies peuvent avoir une incidence négative sur les IPC de performances souhaités.
...
Veillez également à ajouter chaque emplacement à un package avec un type d’objectif de package *[!UICONTROL Prospecting]* ou *[!UICONTROL Retargeting]*, le cas échéant.
...
* **Filtres de pré-enchère :** limitez ou évitez idéalement de définir des filtres de pré-enchère agressifs, ce qui empêche l’emplacement d’atteindre l’échelle. Les bonnes pratiques sont les suivantes :
...
Pour optimiser l’échelle, utilisez [!UICONTROL Public] (Open Exchange) et l’inventaire [!UICONTROL On Demand].
...
* **[!UICONTROL Device Targeting]**:
   ...
   * Ne ciblez pas [!UICONTROL Firefox] et [!UICONTROL Safari] en raison des limites de ciblage et de mesure. Contactez votre gestionnaire de compte d’Adobe pour plus d’informations sur la prise en charge de [!DNL Adobe] pour [!DNL Safari ITP].
...
* Assurez-vous que tous les emplacements contiennent *au moins* toutes les Principales tailles d’annonces d’affichage (300x250, 728x90, 160x600, 300x600, 320x50 et 300x500).
