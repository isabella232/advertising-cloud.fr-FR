...
>[!TIP]
>Utilisez une convention d’affectation des noms adaptée à votre situation. &quot;*\&lt;Nom de campagne\> est l’une des conventions d’appellation suggérées : \&lt;Ad Unit\> : \&lt;Durée\> : \&lt;Ciblage\>*.&quot;
...
**[!UICONTROL Details]:**  (Lecture seule) type de publicité applicable, compte qui crée ou crée l’emplacement et la campagne parente. Pour afficher plus de détails, cliquez sur **[!UICONTROL Show more]**.

**[!UICONTROL Templates]:** ouvre une liste des modèles d’emplacement existants. Pour renseigner automatiquement les paramètres de ciblage à partir d’un modèle :
...
**[!UICONTROL Package]:**  (facultatif). Package auquel l’emplacement est affecté. Cliquez sur ![Modifier](/help/dsp/assets/edit.png) pour sélectionner un package existant ou créer un nouveau package. Lorsque vous attribuez l’emplacement à un package, la section [!UICONTROL Goals] est mise à jour avec les dates de vol, l’objectif de livraison et le budget du package.
...
* *[!UICONTROL Set a fixed budget cap]:* vous permet de définir un budget d’emplacement quotidien, hebdomadaire, mensuel ou à tout moment. Saisissez une valeur et la durée (*[!UICONTROL All time]*, *[!UICONTROL Daily]*, *[!UICONTROL Weekly]*, *[!UICONTROL Monthly]*).

**[!UICONTROL Max Bid]:** maximum de paiement pour 1 000 impressions.

**[!UICONTROL Placement Pre-bid Filters]:** jusqu’à cinq seuils d’indicateurs clés de performance (par exemple, une mesure de visibilité minimale ou un taux de clics publicitaires) qui doivent être atteints pour que l’offre puisse avoir lieu. Vous pouvez utiliser des filtres avant offre comme tactiques d’optimisation, mais vous devez comprendre que chaque règle peut limiter les opportunités sur lesquelles cet emplacement peut enchérir. Pour ajouter ou modifier des filtres :
...
Consultez la description de chaque filtre de pré-offre à l’adresse &quot;[Filtres de pré-offre de niveau placement et Comment les utiliser](/help/dsp/optimization/optimization-pre-bid-filters.md)&quot;.
...
**[!UICONTROL Gross Budget Goal]:**  (Emplacements dans les campagnes avec gestion des marges uniquement) Le budget plafond brut et l’intervalle de budget (*[!UICONTROL All time]*,  *[!UICONTROL Daily]*,  *[!UICONTROL Weekly]*,  *[!UICONTROL Monthly]*).

**[!UICONTROL Optimization Goal]:**  objectif d’optimisation du module. Consultez les descriptions de chaque objectif d’optimisation à l’adresse &quot;[Objectifs d’optimisation et Comment les utiliser](/help/dsp/optimization/optimization-goals.md)&quot;.
...
* **[!UICONTROL Impressions]:** cette option livre des impressions jusqu’à ce qu’une quantité spécifiée soit atteinte au cours d’un intervalle spécifié. Lorsque vous sélectionnez cette option, indiquez le nombre d’impressions et l’intervalle : *[!UICONTROL All time],* *[!UICONTROL Daily],* *[!UICONTROL Monthly],* ou *[!UICONTROL Weekly]*.
...
* *[!UICONTROL Even]:*  (Par défaut) Effectue un espacement uniforme sur chaque vol, avec une cible de 50 % de la diffusion dans la première moitié du vol.

* *[!UICONTROL Frontload]:* accélère la diffusion de sorte qu’elle se termine à 65-75 % à mi-chemin du vol.

* *[!UICONTROL Aggressive Frontload]:* accélère la diffusion de sorte qu’elle soit complète à 75-85 % à mi-chemin du vol.
...
**[!UICONTROL Audience Location]:**  (Facultatif) emplacements spécifiques dans lesquels inclure ou exclure des publicités dans l’emplacement. Si vous ne spécifiez aucun emplacement, tous les emplacements sont ciblés.
...
1. Effectuez l’une des opérations suivantes :
   ...
   * Pour rechercher un code postal et inclure ou exclure tous les résultats sélectionnés :
      ...
      1. Cliquez sur *[!UICONTROL Include All]* pour inclure tous les emplacements comme cibles ou *[!UICONTROL Exclude All]* pour exclure tous les emplacements comme cibles.
...
**[!UICONTROL Inventory Sources]:** sources d’inventaire à inclure ou à exclure en tant que cibles. Pour la plupart des types d’emplacements, tous les types d’inventaire et toutes les sources pour chaque type sont inclus par défaut. Pour les [!DNL Roku] emplacements, vous devez spécifier le type d’inventaire et les sources. Vous pouvez choisir parmi les types de stock suivants :
...
* [!UICONTROL Private] |  [!UICONTROL Roku Private]: Vos offres privées existantes (ou  [!DNL Roku] offres privées existantes pour  [!DNL Roku] des emplacements) avec les éditeurs que vous avez configurés dans DSP. Vous pouvez inclure, mais pas exclure, l’inventaire public.

   Vous pouvez rechercher la liste par mot-clé, clé, identifiant de transaction ou balise personnalisée.

* [!UICONTROL On Demand] |  [!UICONTROL Roku On Demand]: Tous les  [stocks [!UICONTROL On Demand]  non garantis ](/help/dsp/inventory/on-demand-inventory-about.md) Premium (ou  [!UICONTROL On Demand] [!DNL] les offres Roku pour  [!DNL Roku] les emplacements) auxquels vous vous êtes abonné dans  [!DNL DSP]. Vous pouvez inclure et exclure l’inventaire [!UICONTROL On Demand].
...
* Pour cibler un type de stock :
   ...
   1. (Facultatif) Modifiez les sources pour inclure :
      1. Cliquez sur ![Modifier](/help/dsp/assets/edit.png).
      1. ([!UICONTROL Public] et [!UICONTROL On Demand] inventaire) Cliquez sur *[!UICONTROL *View by Source]** ou **[!UICONTROL View by Feed]** pour modifier la façon dont les sources sont répertoriées.
      ...
      1. Indiquez les sources à inclure et à exclure :
         ...
         * Pour inclure des sources [!UICONTROL Private] :
            * Pour inclure tous les stocks d’une transaction, cliquez sur **[!UICONTROL Include all]** en regard de son nom.
         ...
         * Pour exclure une source [!UICONTROL Public] ou [!UICONTROL On ], cliquez sur **[!UICONTROL Exclude]** en regard du nom de la source.
...
>[!TIP]
>
>Si vous vous êtes abonné à l’inventaire [!UICONTROL On Demand] mais que vous ne parvenez pas à localiser les éditeurs ou les offres à cibler, vérifiez l’état des offres. Pour plus d’informations sur les états, voir [À propos [!DNL On Demand] Inventaire Premium](/help/dsp/inventory/on-demand-inventory-about.md).
...
**[!UICONTROL Traffic type]:** les types de trafic à cibler. Les options incluent **[!UICONTROL Websites]** et **[!UICONTROL Apps]**.

**[!UICONTROL Site tier]:**  (Disponible lorsque  **[!UICONTROL Paste list of targeted sites]** est  *[!UICONTROL Off]*) Qualité des sites à cibler. Les niveaux 1 à 3 sont tous sécurisés pour la marque et ont été testés et approuvés par l’équipe de mappage DSP.

* *[!UICONTROL Tier 1]:* sites et applications Premium reconnaissables au plan national.
* *[!UICONTROL Tier 2]:* cible les sites et applications de qualité moins connus que le niveau 1.
...
* *[!UICONTROL All Sites]:* Cible les niveaux 1 à 3 et le nouvel inventaire qui n’a pas été analysé ou classé, que vous pouvez utiliser pour sa portée.
...
**[!UICONTROL Exclude Sites]:**  (facultatif) disponibles lorsque  **[!UICONTROL Paste list of targeted sites]** est  *[!UICONTROL Off]*) Sites à exclure. Vous pouvez rechercher et sélectionner des sites, ou saisir ou coller des noms de domaine :
...
>[!NOTE]
>
>* Les listes de sites bloqués au niveau du compte et de l’annonceur sont également appliquées, en plus de la [liste de sites bloqués au niveau global](/help/dsp/introduction/features/brand-safety-media-quality.md) d’Advertising Cloud DSP, qui inclut les sites considérés comme dangereux pour les publicités.
...
**[!UICONTROL Language]:**  (facultatif) langue à cibler.
...
**[!UICONTROL Included Audiences]:** Toutes les cibles d’audience pour l’emplacement, y compris les segments  [tiers, les segments propriétaires, les segments d’Adobe, les segments personnalisés et les audiences enregistrées](/help/dsp/audiences/audience-settings.md). La taille totale et principale de l’audience dédupliquée pour tous les segments sélectionnés et les audiences enregistrées s’affiche également. Vous pouvez sélectionner une audience existante, créer une audience que vous pourrez réutiliser ultérieurement ou sélectionner des segments d’audience spécifiques :
...
**[!UICONTROL Excluded Audiences]:** toutes les audiences à exclure pour l’emplacement, y compris les audiences avec des segments  [tiers, des segments propriétaires, des segments d’Adobe, des segments personnalisés et des audiences enregistrées](/help/dsp/audiences/audience-settings.md). La taille totale et principale de l’audience dédupliquée pour toutes les audiences exclues s’affiche également. Vous pouvez sélectionner une audience existante ou créer une nouvelle audience que vous pourrez réutiliser ultérieurement :
...
**[!UICONTROL Topic Targeting]:**  (facultatif) Disponibles pour les annonceurs configurés avec  [!DNL Comscore] et  [!DNL Grapeshot] les segments) Noms ou identifiants de segments spécifiques de  [!DNL Comscore] et  [!DNL Grapeshot] à inclure comme cibles. Des frais supplémentaires peuvent s’appliquer pour cette fonctionnalité. Pour activer cette fonction et configurer des segments de rubrique, contactez votre gestionnaire de compte Adobe.
...
**[!UICONTROL Device Targeting]:**  (Facultatif) Informations spécifiques sur les appareils, notamment les types d’appareils, les fabricants, les systèmes d’exploitation, les navigateurs et les types de connectivité, à inclure et à exclure en tant que cibles. Pour spécifier le ciblage des périphériques :
...
* [!UICONTROL Peer 39]:
   ...
   * **Sites Target qui sont :**  (facultatif) un ou plusieurs types d’attributs d’inventaire à cibler par défaut. Des frais supplémentaires peuvent s’appliquer.
...
**[!UICONTROL Pre-bid fraud blocking]:** Types de sites à bloquer en fonction du trafic frauduleux et des activités suspectes mesurées  [!DNL DoubleVerify],  [!DNL Integral Ad Science] et  [!DNL Peer39]. Les valeurs par défaut au niveau de l’annonceur sont sélectionnées pour de nouveaux emplacements, mais vous pouvez modifier les paramètres :
...
* [!UICONTROL Peer 39]:
   ...
   * **[!UICONTROL Block sites that are]:**  (Facultatif) Un ou plusieurs types de fraude qui entraîneront DSP bloquer les publicités par défaut :  *[!UICONTROL Fraud]* (qui bloque tous les sites comportant des fraudes),  *[!UICONTROL Fraud: Bot Sites_Non-Human traffic]* et/ou  *[!UICONTROL Fraud: Zero Ads]*. Des frais supplémentaires peuvent s’appliquer.

* [!UICONTROL Integral Ad Science]:

   * **[!UICONTROL Block sites that are]:**  (Facultatif) Type d’activité de site web ou d’application suspecte qui DSP par défaut bloquer les publicités :  *[!UICONTROL None]* (valeur par défaut, qui ne bloque pas les publicités basées sur des activités suspectes),  *[!UICONTROL Suspicious Activity - High Risk]* ou  *[!UICONTROL Suspicious Activity - High or Moderate Risk]*. Des frais supplémentaires peuvent s’appliquer.

**[!UICONTROL Ads.txt filtering]:**

Quel niveau de [Ads.txt](https://iabtechlab.com/ads-txt-about/) filtrage pré-enchère utiliser en exploitant la liste des vendeurs numériques autorisés de chaque éditeur ? La valeur par défaut au niveau de l’annonceur est sélectionnée pour les nouveaux emplacements, mais vous pouvez modifier les paramètres :
...
**[!UICONTROL DoubleVerify Authentic Brand Safety]:**  (Publicitaires configurés avec l’ [!UICONTROL DoubleVerify Authentic Brand Safety] option) Active  [!DNL DoubleVerify Authentic Brand Safety], qui bloque les impressions après l’offre à l’aide des règles de sécurité de marque personnalisées configurées pour l’identifiant de segment spécifié. DSP facture votre compte pour l’utilisation de l’identifiant de segment spécifié dans les paramètres de l’annonceur.

## [!UICONTROL Tracking]

>[!NOTE]
>
>([!DNL Roku] emplacements) Les fournisseurs tiers de suivi approuvés par [!DNL Roku] comprennent [!DNL Acxiom], [!DNL comScore], [!DNL Data Plus Math], [!DNL Experian], [!DNL Factual], [!DNL Kantar], [!DNL Marketing Evolution], [!DNL Neustar], [!DNL Nielsen], [!DNL Nielsen Catalina Solutions], [!DNL NinthDecimal], &lt;a111111111 3/>, [!DNL Placed], [!DNL Polk] et [!DNL Research Now].[!DNL Oracle]
...
1. Effectuez l’une des opérations suivantes :
   ...
   * Pour créer un pixel :
      ...
      1. Renseignez les informations suivantes :
         ...
         * **[!UICONTROL Notes]:**  (facultatif) description ou autres informations sur le pixel.
      1. Cliquez sur **[!UICONTROL Create and attach]**.
...
**[!UICONTROL 3rd-party Fees]:**  (Facultatif) Taux de frais tiers statique à suivre en tant que coût non facturable par millier d’impressions. Le cas échéant, la valeur par défaut au niveau du package est automatiquement appliquée pour les nouveaux emplacements, sauf si vous saisissez une autre valeur.
